        ...
        if (rows.length > 1) {
            ...
            for (let index = 0; index < rows.length; index++) {
                ...
                if (rows[index].Key == "MUL") {
                    ...
                    let SQL = "SELECT i.TABLE_SCHEMA, i.TABLE_NAME, i.CONSTRAINT_TYPE, i.CONSTRAINT_NAME, k.REFERENCED_TABLE_NAME, k.REFERENCED_COLUMN_NAME FROM " +
                    ...
                    ForeignKeyDetect(SQL, queryObject, closed, (result) => {
                        console.log(result);
                        object_mappe.properties[index].isForeignKey = result;
                        readerDB.writeFile('mappage.json', JSON.stringify(object_mappe));
                    })*/
    ...
    readerDB.appendFile('models/models.js', data, (err) => {
        ...
        if (closed) {
            ...
            ForeignKeyDetect(TabList)
                // console.log('Table Object :', JSON.stringify(TabList));
...
let ForeignKeyDetect = (tab) => {
    ...
    for (var index = 0; index < TabList.length; index++) {
        ...
        let arrayColumns = [];
        let columnObject = {};
        //  maxCols = TabList[index].properties.length;
        let objectFK = {
        ...
        if (arrayColumns.length > 0) {
            infoFK(SQL, objectFK, TabList[index], arrayColumns, closed, (response) => {
                mapData.push(response);
                readerDB.writeFile('./mappage.json', JSON.stringify(mapData));
    ...
    /*t.properties.forEach((elt) => {
        if (elt.isForeignKey != undefined) {


            queryExecute.query(SQL, [objectFK.criteria, objectFK.database, objectFK.table], (err, rows, fields) => {
                console.log('Sizer Mapping :', rows.length)
            });
        }
...
let infoFK = (SQL, objectFK, Table, Cols, closed, cb) => {
        queryExecute.query(SQL, [objectFK.criteria, objectFK.database, objectFK.table], (err, rows, fields) => {
            ...
            (closed == true) ? queryExecute.end(): false;
            cb(Table);
    ...
    queryExecute.query(SQL, [parameter.status, parameter.database, parameter.table], (err, rows, fields) => {

        console.log('Sizer :', parameter);
        if (rows.length == 1) {
            console.log('Table :' + rows)
            let ForeignKeyObject = {
                "table": rows[0].REFERENCED_TABLE_NAME,
                "idKey": rows[0].REFERENCED_COLUMN_NAME,
                "fk_name": rows[0].CONSTRAINT_NAME
            }
            if (closed) {
                queryExecute.end();
            }
            ...
            console.log('ForeignKey not found');
        }
    })
}*/
